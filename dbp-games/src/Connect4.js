...
module.exports = class Connect4Game {
    constructor(options = {}) {
        ...
        if (!options.embed.title) options.embed.title = '4 pod rząd';
        if (typeof options.embed.title !== 'string')  throw new TypeError('INVALID_TITLE: Embed Title must be a string.')
        if (!options.embed.color) options.embed.color = '#1eff00';
        ...
        if (!options.askMessage) options.askMessage = 'Cześć {opponent}, {challenger} Wyzywa cię na pojedynek **4 pod rząd**!';
        ...
        if (!options.turnMessage) options.turnMessage = ' Teraz kolej gracza **{player}**.';
        if (typeof options.turnMessage !== 'string')  throw new TypeError('TURN_MESSAGE: Turn Message must be a string.')
        if (!options.waitMessage) options.waitMessage = 'To nie twoja kolej!';
    ...
    async startGame() {
        ...
        if (this.opponent.id === this.message.author.id) return this.sendMessage('<a:nie:957694133596737606> Nie mozesz zagrać z sasmym sobą!')
    ...
    GameEmbed() {
        ...
        .addField(this.options.embed.statusTitle || 'Status Graczy', status)
