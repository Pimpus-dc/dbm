...
module.exports = class Trivia {
	...
	async startGame() {
      ...
      data.correct = shuffled.indexOf(result.correct_answer)
		...
		const btr1 = new MessageButton().setStyle('PRIMARY').setCustomId('1_trivia').setLabel(data.options[0])
		const btr2 = new MessageButton().setStyle('PRIMARY').setCustomId('2_trivia').setLabel(data.options[1])
		const btr3 = new MessageButton().setStyle('PRIMARY').setCustomId('3_trivia').setLabel(data.options[2])
		const btr4 = new MessageButton().setStyle('PRIMARY').setCustomId('4_trivia').setLabel(data.options[3])
		const row = new MessageActionRow().addComponents(btr1, btr2, btr3, btr4)
