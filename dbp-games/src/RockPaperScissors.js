...
module.exports = class RPSGame {
    constructor(options = {}) {
        ...
        if (!options.embed.color) options.embed.color = '#e5ff00';
        ...
        if (!options.askMessage) options.askMessage = 'Cześć {opponent}, {challenger} Wyzywa cię na pojedynek kamień papier nożyce!';
        if (typeof options.askMessage !== 'string')  throw new TypeError('ASK_MESSAGE: Ask Messgae must be a string.')
        if (!options.cancelMessage) options.cancelMessage = 'Odrzucono prośbę o zagranie.';
        if (typeof options.cancelMessage !== 'string')  throw new TypeError('CANCEL_MESSAGE: Cancel Message must be a string.')
        if (!options.timeEndMessage) options.timeEndMessage = '**Brak odpowiedzi przez użytkownika!**';
        if (typeof options.timeEndMessage !== 'string')  throw new TypeError('TIME_END_MESSAGE: Time End Message must be a string.')
        
        
        if (!options.othersMessage) options.othersMessage = '<a:nie:957694133596737606> Nie posiadasz uprawnień!';
        ...
        if (!options.noChangeMessage) options.noChangeMessage = '<a:nie:957694133596737606>  Nie mozesz zmienić wyboru!';
        ...
        if (!options.winMessage) options.winMessage = '🎉 Gratulacje {winner} wygranej! ';
    ...
    async startGame() {
        ...
        if (this.opponent.bot) return this.sendMessage('<a:nie:957694133596737606> Nie mozesz zagrać z botem!')
        if (this.opponent.id === this.message.author.id) return this.sendMessage('<a:nie:957694133596737606> Nie mozesz zagrać z samym sobą!')
